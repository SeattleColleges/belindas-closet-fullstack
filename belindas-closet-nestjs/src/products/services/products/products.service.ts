...
import { sanitizeForLogging, sanitizeProductForLogging } from '../../../utils/logging-sanitizer';
...
export class ProductsService {
  ...
  async createProduct(product: Partial<Product>, user: User): Promise<Product> {
    this.logger.log(
      `Creating Product: ${JSON.stringify(sanitizeProductForLogging(product), null, '\t')}`,
      this.SERVICE,
    );
  ...
  async findOne(id: string): Promise<Product> {
    ...
    this.logger.log(
      `Product found: ${JSON.stringify(sanitizeProductForLogging(product), null, '\t')}`,
      this.SERVICE,
    );
    return product;
  }
