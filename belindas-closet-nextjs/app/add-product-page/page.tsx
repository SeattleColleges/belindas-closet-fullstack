...
const URL = process.env.NEXT_PUBLIC_BELINDAS_CLOSET_PUBLIC_API_URL || 'http://localhost:3000/api';
...
const AddProduct = ({ categoryId }: { categoryId: string }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    // Prepare data according to backend DTO requirements - only send fields that have values
    ...
    // Only add size fields if they have values, and ensure they're strings
    if (formData.productSizeShoe && formData.productSizeShoe !== "") {
      const sizeValue = String(formData.productSizeShoe).trim();
      if (sizeValue !== "") {
        processedFormData.productSizeShoe = sizeValue;
      }
    }
    if (formData.productSizes && formData.productSizes !== "") {
      const sizeValue = String(formData.productSizes).trim();
      if (sizeValue !== "") {
        processedFormData.productSizes = sizeValue;
      }
    }
    if (formData.productSizePantsWaist && formData.productSizePantsWaist !== "") {
      const sizeValue = String(formData.productSizePantsWaist).trim();
      if (sizeValue !== "") {
        processedFormData.productSizePantsWaist = sizeValue;
      }
    }
    if (formData.productSizePantsInseam && formData.productSizePantsInseam !== "") {
      const sizeValue = String(formData.productSizePantsInseam).trim();
      if (sizeValue !== "") {
        processedFormData.productSizePantsInseam = sizeValue;
      }
    }
    if (formData.productDescription && formData.productDescription !== "") {
      const descValue = String(formData.productDescription).trim();
      if (descValue !== "") {
        processedFormData.productDescription = descValue;
      }
    }
    if (formData.productImage && typeof formData.productImage === 'string' && formData.productImage.trim() !== "") {
      processedFormData.productImage = String(formData.productImage);
    }
              ...
              {filteredProducts.map((product, index) => (
                <Grid item key={index} xs={12} sm={4} md={3} sx={{ marginRight: { xs: 4} }}>
                  <ProductCard
                    ...
                    href={`/category-page/${categoryId}/products/${product.id}`}
                    id={product.id}
                    isHidden={false}
                    isSold={false}
                  />
